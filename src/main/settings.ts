...
const IS_WEB_MODE = process.env.DYAD_WEB_MODE === "true";
const DEFAULT_WEB_RUNTIME =
  process.env.DYAD_WEB_DEFAULT_RUNTIME === "docker" ||
  process.env.DYAD_WEB_DEFAULT_RUNTIME === "host"
    ? process.env.DYAD_WEB_DEFAULT_RUNTIME
    : undefined;
...
const DEFAULT_SETTINGS: UserSettings = {
  ...
  runtimeMode2: DEFAULT_WEB_RUNTIME,
};
...
export function readSettings(): UserSettings {
  try {
    ...
    if (IS_WEB_MODE && !validatedSettings.runtimeMode2 && DEFAULT_WEB_RUNTIME) {
      validatedSettings.runtimeMode2 = DEFAULT_WEB_RUNTIME;
    }
